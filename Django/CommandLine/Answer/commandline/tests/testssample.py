...
class EditTest(TestCase):
    def setUp(self):
        Company.objects.create(name='Quera', email="quera@email.com", phone="09122201202",
                               description="a place for programmers")

    def test_success_edit(self):
        try:
            management.call_command('editCompany', 'Quera', name='quera', email="Quera@email.com",
                                    phone="+989112201202", description="Programming Contest")
        except:
...
class RemoveTest(TestCase):
    def setUp(self):
        self.quera = Company.objects.create(name='Quera', email="quera@email.com", phone="09122201202",
                                            description="a place for programmers")
